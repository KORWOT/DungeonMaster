...
namespace DungeonMaster.Battle
{
    ...
    {
        [Header("전투 설정")]
        [SerializeField] private DeterministicBattleSettingsData battleSettings;
        ...
        /// <summary>
        /// 전투를 시작합니다.
        /// DungeonData를 받아 전투의 모든 과정을 관리합니다.
        /// </summary>
        public void StartBattle(DungeonData dungeonData)
        {
            // 0. 의존성 확인
            ...
            var damageSettings = Resources.Load<DamageSettings>("Battle/DamageSettings");
            var gradeGrowthConfig = Resources.Load<GradeGrowthConfig>("Character/GradeGrowthConfig");

            if (elementalTable == null || damageSettings == null || gradeGrowthConfig == null)
            {
                Debug.LogError("핵심 설정 파일(Elemental, Damage, Growth)을 찾을 수 없습니다. Resources 폴더를 확인하세요.");
                return;
            }

            _battleRules = new DeterministicBattleRules(battleSettings, seed, elementalTable, damageSettings);
            ...
            // 플레이어 캐릭터 생성
            for (int i = 0; i < dungeonData.PlayerMonsters.Count; i++)
            {
                ...
                var characterData = CharacterDataFactory.Create(
                    dungeonData.PlayerMonsters[i].Blueprint, 
                    dungeonData.PlayerMonsters[i].UserCard, 
                    gradeGrowthConfig, 
                    true, // isPlayer
                    GetNextInstanceId()
                );
            ...
            // 적 캐릭터 생성
            for (int i = 0; i < dungeonData.EnemyMonsters.Count; i++)
            {
                ...
                var characterData = CharacterDataFactory.Create(
                    dungeonData.EnemyMonsters[i].Blueprint, 
                    dungeonData.EnemyMonsters[i].UserCard, 
                    gradeGrowthConfig, 
                    false, // isPlayer
                    GetNextInstanceId()
                );
